...
class KITTI_Vision(Dataset):
    """
        KITTI Vision object det dataset.
    Note:
        using YOLO label format
        https://github.com/pjreddie/darknet
    Example:
    ...
    def __init__(self, root_dir, img_width=None, img_height=None):
        """

        :param txt_file: all image directories
        """
        image_dir_path = os.path.join(root_dir, 'training/image_2')
        label_dir_path = os.path.join(root_dir, 'training/label_2')
        samples = os.listdir(image_dir_path)
        self.image_list = samples
        self.label_list = [filename.replace('.png', '.txt')
    ...
    def __getitem__(self, idx):
        ...
        if self.img_width and self.img_height:
            image = resize(image, (self.img_width, self.img_height))
            image = torch.Tensor(image).permute(2, 0, 1)  # pytorch format: C W H  # TODO
